...
- Toolkit state: independent public preview licensed under UPL-1.0 and maintained
  ...
  Follow-up docs and evidence on `main` retain version `0.1.0` without changing
  the release tag or its assets.
...
- Conciseness and progressive disclosure for focused prompts. The fresh native
  response was safe and fixture-grounded but longer than needed; tighter
  answers and minimal reference loading need a separately reviewed skill change.
